    ...
    public static class XpandModelEditor {
        ...
        private static int ExtractME() {
            ...
            if (!File.Exists(zipPath)) {
                ...
                var exeDir = $"{MESettingsPath}\\Xpand.XAF.ModelEditor.Win";
                var vsixPath = Path.GetDirectoryName(typeof(XpandModelEditor).Assembly.Location);
                foreach (var dll in new[] { "Microsoft.VisualStudio.SolutionPersistence.dll", "Microsoft.IO.Redist.dll" }) {
                    File.Copy(Path.Combine(vsixPath, dll), Path.Combine(exeDir, dll), overwrite: true);
                }
